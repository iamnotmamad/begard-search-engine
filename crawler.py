...
CRAWL_MODE = 1   # 1=internal first, 2=all equal, 3=discover new sites, 4=recrawl indexed
...
def crawl_single(url_row):
    ...
    for link in new_links:
        ...
        with url_cache_lock:
            ...
            elif CRAWL_MODE == 3:        # کشف سایت‌های جدید
                if link_domain == domain:
            ...
            elif CRAWL_MODE == 4:        # خزش مجدد سایت‌های ایندکس‌شده
    ...
    log.crawl(f"{final_url} (d={depth}) +{len(new_links)}L, {added} new")
    db.set_link_status(original_url, 'done')
    update_queue_file()
...
def run_infinite_crawler():
    ...
    log.info(f"Crawler started ({CRAWLER_THREADS} workers) [Mode {CRAWL_MODE}: {mode_names.get(CRAWL_MODE, 'Unknown')}]...")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 3 and sys.argv[1] == '--feeds':
        from feed_loader import load_feed
        load_feed(sys.argv[2])
    elif os.path.exists(DEFAULT_FEED):
